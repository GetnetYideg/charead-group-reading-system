...
export const register = async (req, res) => {
    try {
        ...
        if (error) throw new Error("somthing bad happened, in the db :" + error.message)
...
export const githubOauthCallback = async (req, res) => {
    ...
    try {
        ...
        // console.log(githubUser.data.name + " this is user");
...
export const googleOauthCallback = async (req, res) => {
    ...
    try {
        ...
        // Look up existing user by oauth_provider_id (most reliable for returning users)
        const { data: userFromDB } = await supabase
            .from('User')
            .select('*')
            .eq('oauth_provider_id', googleUser.data.sub)
            .limit(1)

        if (userFromDB && userFromDB.length > 0) {
            // Existing user — sign them in
            const existingUser = { ...user, id: userFromDB[0].id, username: userFromDB[0].username }
            const token = jwt.sign(existingUser, process.env.SECRET_KEY, { expiresIn: "1d" })
            return res.cookie('token', token, {
                httpOnly: true,
                secure: process.env.NODE_ENV === 'production',
                sameSite: 'lax',
            ...
            // New user — create account with unique username
            const username = googleUser.data.email.split('@')[0] + randomNDigits(5)
            const newUser = { ...user, username }
            const { data, error } = await supabase
                .from("User")
                .insert([newUser]).select()
            if (error) throw new Error(error.message)
            const token = jwt.sign({ ...newUser, id: data[0].id }, process.env.SECRET_KEY, { expiresIn: "1d" })
            return res.cookie('token', token, {
                httpOnly: true,
                secure: process.env.NODE_ENV === 'production',
                sameSite: 'lax',
